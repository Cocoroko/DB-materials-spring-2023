...
-- cte syntax: with {name} as ()
-- with {name1} as (), {name2} as () ...
WITH ranks as (
SELECT
competition_nm,
result_sorting_type_code,
ROW_NUMBER() OVER w_desc as desc_rank,
ROW_NUMBER() OVER w_asc as asc_rank,
participant_result_amt
FROM competition
JOIN competition_result
USING (competition_id)
	WINDOW
	w_desc as (PARTITION BY competition_id ORDER BY participant_result_amt DESC),
	 w_asc as (PARTITION BY competition_id ORDER BY participant_result_amt ASC)
)
SELECT competition_nm, participant_result_amt, result_sorting_type_code
FROM ranks
WHERE
(desc_rank = 1 AND result_sorting_type_code='DESC')
OR (asc_rank = 1 AND result_sorting_type_code='ASC');


-- Для каждого соревнования вывести призовые места
-- Почему тут не ROW_NUMBER?

WITH ranks as (
SELECT
competition_nm,
result_sorting_type_code,
DENSE_RANK() OVER w_desc as desc_rank,
DENSE_RANK() OVER w_asc as asc_rank,
participant_result_amt,
participant_id
FROM competition
JOIN competition_result
...
SELECT competition_nm, participant_result_amt, participant_nm, result_sorting_type_code,
CASE
WHEN result_sorting_type_code='DESC' THEN desc_rank
WHEN result_sorting_type_code='ASC' THEN asc_rank
END AS rank
FROM ranks
JOIN participant
USING (participant_id)
WHERE
(desc_rank <= 3 AND result_sorting_type_code='DESC')
OR (asc_rank <= 3 AND result_sorting_type_code='ASC')
ORDER BY competition_nm, result_sorting_type_code, rank;
...
(
	...
	competition_nm,
	participant_result_amt,
	CASE WHEN result_sorting_type_code='DESC' THEN
	DENSE_RANK() OVER (PARTITION BY competition_id ORDER BY participant_result_amt DESC)
	WHEN result_sorting_type_code='ASC' THEN
	DENSE_RANK() OVER (PARTITION BY competition_id ORDER BY participant_result_amt ASC)
	END
	AS rank,
	CASE WHEN result_sorting_type_code='DESC' THEN
	FIRST_VALUE(participant_result_amt)
	OVER (PARTITION BY competition_id ORDER BY participant_result_amt DESC)
	WHEN result_sorting_type_code='ASC' THEN
	FIRST_VALUE(participant_result_amt) OVER (PARTITION BY competition_id ORDER BY participant_result_amt ASC)
	END AS top_score
	FROM competition
	JOIN competition_result
	USING (competition_id)
...
SELECT
competition_nm,
participant_result_amt - top_score AS score_dif
FROM t
WHERE rank > 1;
...
SELECT
	...
	participant_nm,
	CASE WHEN result_sorting_type_code='DESC' THEN
	FIRST_VALUE(participant_birth_dt)
	OVER (PARTITION BY competition_id ORDER BY participant_result_amt DESC)
	WHEN result_sorting_type_code='ASC' THEN
	FIRST_VALUE(participant_birth_dt) OVER (PARTITION BY competition_id ORDER BY participant_result_amt ASC)
	END AS winner_bday,
participant_result_amt,
participant_id
...
SELECT competition_nm, participant_nm
FROM t
WHERE participant_birth_dt > winner_bday;
...
SELECT
	...
	participant_id,
	participant_result_amt,
	LEAD(participant_result_amt)
	OVER (PARTITION BY competition_id ORDER BY participant_result_amt)
	AS next_result,
	LAG(participant_result_amt)
	OVER (PARTITION BY competition_id ORDER BY participant_result_amt)
	AS prev_result
FROM competition_result
)
SELECT
*,
participant_result_amt - next_result AS next_dif,
participant_result_amt - prev_result AS prev_dif
FROM t;
...
--prerequisites: union, intersect, except
...
insert into person_1 values
('Mike', 1), ('Nelli', 2), ('Max', 3), ('Ilia', 4);

insert into person_2 values
('Esya', 1), ('Nelli', 2), ('Nikita', 3), ('Max', 4);

--(reminder)about values operator:
...
UNION ALL
SELECT 3, 'three';
--details here https://www.postgresql.org/docs/current/sql-values.html


select name from person_1 union  select name from person_2;
--here we can use union all to add all duplicates

select name from person_1 union select name from person_2 limit 2;
--it means (select name from person_1 union select name from person_2) limit 2;

select name from person_1 intersect select name from person_2;

select name from person_1 except select name from person_2;

select name, id_pers from person_1 except select name from person_2;
--error, because different number of columns
--for more details https://www.postgresql.org/docs/current/queries-union.html
...
CREATE TABLE geo_tree (
    ...
    parent_id int references geo_tree(my_id), --it is self-reference, see here https://www.postgresql.org/docs/current/ddl-constraints.html
    name varchar(100)
);

insert into geo_tree  (my_id, parent_id, name)
values
...
WITH RECURSIVE r AS (
   SELECT my_id, parent_id, name
   FROM geo_tree
   WHERE parent_id = 4

   UNION

   SELECT my_id, parent_id
   FROM geo_tree
   WHERE parent_id IN (
      SELECT my_id FROM r
   )
...
--we can't use subqueries in recursion. use join instead.
-- for more examples about recursive with see https://www.postgresql.org/docs/current/queries-with.html
